...
class NamedHyphenator(pyphen.Pyphen):
    ...
    def __init__(
        self, filename=None, lang=None, left=1, right=2, cache=True, name="[UNNAMED HYPHENATOR]"
    ):
...
# Skipta from 1985 (built into Pyphen)
hyphenator_unchanged = NamedHyphenator(
    lang="is_1985_unchanged_list", name="New patterns from 1985 list"
)
...
hyphenator_with_more_additions = NamedHyphenator(
    lang="is_2020_alpha2_extra", name="Corrected with more additions"
)
...
print(
    "\t".join(
        ["HYPHENATOR", "Perfect words", "Okay words", "Bad words", "Good hyphens", "Bad hyphens"]
    )
)
...
for hyphenator in hyphenators:
    ...
    output_file.write(
        f"Hyphenating words according to {hyphenator.name}; showing errors below.\n"
    ...
    for word in test_words:
        ...
        if not good_points == all_points:
            ...
            output_file.write(
                standard_hyphenated_words[word]
                + "\t"
                + hyphenator.inserted(word, hyphen="-")
                + "\n"
            )
